...
class FavouritePageController {
  ...
  FavouritePageController({required this.context}) {
    databaseProvider = Provider.of<DatabaseProvider>(context, listen: false);
  }

  Future getFavourites() async {
    databaseProvider.favouriteList =
        await TranslateDataBase.instance.readFavourite();
  }

  Future deleteFavourite(Favourite favourite) async {
    bool existHistory =
        await TranslateDataBase.instance.getSingleHistory(favourite.historyId!);
    print(existHistory);
    if (existHistory) {
      print('update');
      ...
      final update = History(
          ...
          isFavorite: 'false');
      await TranslateDataBase.instance
          .deleteFavouriteField(favourite.id)
          .then((res) async {
        await TranslateDataBase.instance
            .updateHistory(update)
            .then((value) async {
          databaseProvider.historyList =
              await TranslateDataBase.instance.readHistory();
          databaseProvider.favouriteList =
              await TranslateDataBase.instance.readFavourite();
        });
      });
      ...
      await TranslateDataBase.instance
          .deleteFavouriteFieldHistoryId(favourite.historyId)
          .then((res) async {
        databaseProvider.favouriteList =
            await TranslateDataBase.instance.readFavourite();
      });
