...
class DraggablePanel extends StatefulWidget {
  ...
  /// Allows toggling of the draggability of the DraggablePanel.
...
class _DraggablePanelState extends State<DraggablePanel>
            ...
            onVerticalDragEnd: widget.backdropTapClosesPanel
                ? (DragEndDetails data) {
                    ...
                    if ((widget.slideDirection == SlideDirection.up ? 1 : -1) *
                            data.velocity.pixelsPerSecond.dy >
                        0) _close();
                  }
                : null,
  ...
  void _onGestureSlide(double dy) {
    ...
    if (_isPanelOpen && _sc.hasClients && _sc.offset <= 0) {
      setState(() {
        /// if SlideDirection.UP scroll direction will be down
        if (widget.slideDirection == SlideDirection.up) {
          if (dy < 0) {
        ...
        /// else SlideDirection.UP scroll direction will be up
        else {
          if (dy < 0) {
            _scrollingEnabled = false;
          } else {
  ...
  void _onGestureEnd(Velocity v) {
    ...
    /// large value if null results in not every being the min
...
class PanelController {
  _DraggablePanelState? _panelState;

  void _addState(_DraggablePanelState panelState) {
    _panelState = panelState;
  }

  /// Determine if the panelController is attached to an instance
  /// of the DraggablePanel (this property must return true before any other
  /// functions can be used)
  bool get isAttached => _panelState != null;

  /// Closes the sliding panel to its collapsed state (i.e. to the  minHeight)
  Future<void> close() {
    assert(isAttached, "PanelController must be attached to a DraggablePanel");
  ...
  /// Requires that the DraggablePanel snapPoint property is not null
  /// (optional) duration specifies the time for the animation to complete
  /// (optional) curve specifies the easing behavior of the animation.
  Future<void> animatePanelToSnapPoint(
    ...
    assert(_panelState!.widget.snapPoint != null,
        "DraggablePanel snapPoint property must not be null");
    return _panelState!
